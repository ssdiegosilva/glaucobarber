...
export async function GET(req: NextRequest) {
  ...
  after(async () => {
    ...
    const run   = await prisma.cronRun.create({ data: { cronName: "vitrine-publish", status: "running" } });
    ...
    try {
      ...
      for (const post of scheduled) {
        if (post.images.length === 0 || !post.caption?.trim()) {
          await prisma.vitrinPost.update({ where: { id: post.id }, data: { status: "FAILED", errorMsg: "Post sem fotos ou legenda" } });
          failed++;
          continue;
        }
        ...
        if (!integration?.instagramPageAccessToken || !integration.instagramBusinessId) {
          await prisma.vitrinPost.update({ where: { id: post.id }, data: { status: "FAILED", errorMsg: "Instagram não configurado" } });
          failed++;
          continue;
        }
        ...
        try {
          ...
          await prisma.vitrinPost.update({
            where: { id: post.id },
            data: { status: "PUBLISHED", publishedAt: now, instagramPostId: postId, instagramPermalink: permalink },
          });
          published++;
        } catch (err) {
          console.error(`[cron/vitrine-publish] Failed ${post.id}:`, err);
          await prisma.vitrinPost.update({
            where: { id: post.id },
